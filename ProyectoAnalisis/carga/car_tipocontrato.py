...
def cargar_tipocontrato():
    try:
        ...
        consulta_ext_tipo_contrato = "SELECT tipoContratoID, nombre FROM ext_tipocontrato"
        df_ext_tipo_contrato = pd.read_sql(consulta_ext_tipo_contrato, con=ses_db_stg)

        # Crear una tabla temporal en la base de datos de destino
        create_temp_table = """
        CREATE TEMPORARY TABLE temp_ext_tipo_contrato AS (
            SELECT tipoContratoID, nombre
            FROM dim_tipocontrato
        )
        """
        # Actualizar los registros existentes en la tabla de dimensión (TipoContrato)
        update_query = """
        UPDATE dim_tipocontrato AS tc
        JOIN temp_ext_tipo_contrato AS etc ON tc.tipoContratoID = etc.tipoContratoID
        SET tc.nombre = etc.nombre
            
        """
        # Insertar registros nuevos en la tabla de dimensión (TipoContrato)
        insert_query = """
        INSERT INTO dim_tipocontrato (tipoContratoID, nombre)
        VALUES (%s, %s)
        """
        ...
        with ses_db_sor.begin() as conn:
            ...
            records = df_ext_tipo_contrato.to_dict(orient='records')
